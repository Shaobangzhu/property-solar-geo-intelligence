...
## Current scope: M1 Property Search

The React/Vite frontend checks a local PostgreSQL property registry first, then uses ArcGIS stored geocoding for a new address. It saves the address and coordinates through the Express API and shows the Property Summary. The map/3D scene and PVWatts production modeling are planned for later milestones.
...
Set `server/.env` `DATABASE_URL` to the local PostgreSQL connection string. Set `client/.env` `VITE_ARCGIS_API_KEY` to an ArcGIS API key with **stored geocoding** privilege. `PVWATTS_API_KEY` is reserved for a later backend-only integration and is not used in M1. The two `.env` files are ignored by Git.
